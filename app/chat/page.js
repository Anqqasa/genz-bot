...
export default function Home() {
  ...
  const captureScreenshot = async (e, msgIndex) => {
    ...
    const botMsg = messages[msgIndex].content;
    setShareData({ userMsg, botMsg });
    
    setTimeout(async () => {
      const shareElement = document.getElementById('share-card-template');
      if (shareElement) {
        try {
          const canvas = await html2canvas(shareElement, { backgroundColor: '#09090b', scale: 2 });
          
          if (navigator.share) {
             canvas.toBlob(async (blob) => {
                const file = new File([blob], 'sipaling-ai-roast.png', { type: 'image/png' });
                try {
                  await navigator.share({
                    title: 'SiPaling.AI Roast',
                    text: 'Cek roastingan ini dari SiPaling.AI wkwk!',
                    files: [file]
                  });
                } catch(err) {
                  const link = document.createElement('a');
                  link.download = `sipaling-ai-${Date.now()}.png`;
          ...
          <div id="share-card-template" className="share-card-container">
            ...
            {shareData.userMsg && (
              <div className="share-card-user">
                ...
                <p>{shareData.userMsg}</p>
              </div>
            )}
            
            <div className="share-card-bot">
              <ReactMarkdown remarkPlugins={[remarkGfm]}>{shareData.botMsg}</ReactMarkdown>
            </div>
            
            <div className="share-card-footer">
              <span>sipaling-ai.vercel.app</span>
